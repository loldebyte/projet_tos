...
bool test_validate_input(void) {
    {
        ...
        assert(assert_valid_input(&valid_string) && "String validation of valid string failed\n");
        int len = strlen(valid_string);
        assert(valid_string[len-1] == 0 && "trailing \\n not removed !\n");
    }
    {
        char too_short_string[BUFFER] = "a\n";
        assert(assert_valid_input(&too_short_string) == false
    ...
    }
    {
        ...
        assert(assert_valid_input(&too_long_string) == false
               && "Invalid string didnt fail !");
    }
}
